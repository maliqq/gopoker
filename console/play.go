...
// Reply - reply to play
func (play *Play) Send(msg message.Message) {
  play.Instance.DealProcess.Recv <- event.New(msg)
}

func (play *Play) Start(recv event.Channel) {
  for n := range recv {
    ...
    case *message.RequireBet:
      ...
      pos := msg.Pos
      seat := play.Instance.Table.Seat(pos)
      ...
      if newBet != nil {
        play.Send(&message.AddBet{pos, newBet})
      }
    ...
    case *message.RequireDiscard:
      ...
      play.Send(&message.DiscardCards{pos, cards})
    ...
    case *message.AddBet:
      ...
      pos := msg.Pos
      player := play.Instance.Table.Player(pos)
